...
TEST_F(MoneyManagerTest, InvalidIncreaseTest) {
	...
	EXPECT_THROW(money_manager.Increase((PlayerId::PLAYER1), 0), std::out_of_range);

	EXPECT_THROW(money_manager.Increase((PlayerId::PLAYER1), -5000), std::out_of_range);
}
...
TEST_F(MoneyManagerTest, InvalidDecreaseTest) {
	// Test for zero increase
	EXPECT_THROW(money_manager.Decrease((PlayerId::PLAYER1), 0), std::out_of_range);

	// Test for Negative Amount
	EXPECT_THROW(money_manager.Decrease((PlayerId::PLAYER1), -5000), std::out_of_range);

	// Test for Insufficient Balance
	EXPECT_THROW(money_manager.Decrease(PlayerId::PLAYER1, 20000), std::out_of_range);
	ASSERT_EQ(money_manager.GetBalance(PlayerId::PLAYER1), 5000);
}
